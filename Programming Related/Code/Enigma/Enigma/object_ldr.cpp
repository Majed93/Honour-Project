...
Modified by Majed Monem to include textures
...
{
	colours = new GLfloat[vertices.size()*sizeof(glm::vec3) * 4];
	
	textures = new GLfloat[vertices.size()*sizeof(glm::vec3) * 2];
	...
	glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(glm::vec3), &(colours[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	...
	glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(glm::vec3), &(textures[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void object_ldr::drawObject()
{
	...
	glVertexAttribPointer(
		attribute_v_colours,  // attribute index
		3,                  // number of elements per vertex, here (x,y,z)
		GL_FLOAT,           // the type of each element
		GL_FALSE,           // take our values as-is
		0,                  // no extra data between each position
